# CSDF: Cognitive Self-Feedback Data Framework (update 21/06/2026)
...
On prior art — what's borrowed vs. what's added: the basic mechanism of training an upstream component using a frozen downstream model's performance as the supervision signal is not new. REPLUG LSR (2023) trains a retriever this exact way, using a frozen language model's perplexity as the training signal. CSDF does not claim to have invented that mechanism. What CSDF adds on top of it: generalizing from a single fixed retriever/LM pair to an arbitrary number of self-selecting specialists; sub-task ownership for structural error localization across that many-specialist system; combining the signal with a self-model that crystallizes through experience (inward memory); and treating the whole thing as a general architectural principle for multi-agent coherence, rather than a training trick specific to retrieval.
...
| **REPLUG LSR** (2023) | The core training-signal mechanism itself — an upstream component trained using a frozen downstream model's performance as supervision | Generalizes from a fixed retriever/LM pair to an arbitrary number of self-selecting specialists; adds sub-task ownership for structural error localization; combines the signal with a self-model that crystallizes through experience; treats it as a general multi-agent coherence principle rather than a retrieval-specific training trick |



**The gap CSDF addresses:** existing work does memory architecture, or self-improving flywheels, or Hebbian-style graph updates, or neurobiologically-inspired non-parametric retrieval, or trace-driven component optimization, or — closest of all — downstream-frozen-model performance as an upstream training signal in a single fixed pair (REPLUG LSR) — each in isolation. No existing system combines that training-signal mechanism with self-selecting multi-agent structure, sub-task ownership for structural error localization, training targets that emerge from operation across an arbitrary number of specialists, and a self-model that crystallizes through experience — as one explicit architectural principle for emergent multi-agent coherence.
